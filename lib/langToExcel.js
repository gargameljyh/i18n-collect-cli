  ...
  for (let i = 0; i < codeArr.length; i++) {
    ...
    if (code.indexOf("//") >= 0 || code.indexOf("/*") >= 0) {
      if (code.indexOf("//") === 0 || code.indexOf("/*") >= 0) {
        ...
        // todo /* 注释标识符判断
        codeArr[i] = codeArr[i].slice(
          0,
          codeArr[i].indexOf(code.slice(code.indexOf("//")))
        );
      }
    }
  }
...
const objToLink = (list, obj, translateUrl, traJson, linkStr = "") => {
  for (let lkey in obj) {
    if (typeof obj[lkey] === "string") {
      if (translateUrl && get(traJson, `${lkey}`)) {
        list.push([`${lkey}`, obj[lkey], get(traJson, `${lkey}`)]);
      } else {
        list.push([`${lkey}`, "", "", "", "", "", obj[lkey]]);
      }
    }
    if (typeof obj[lkey] === "object") {
      objToLink(list, obj[lkey], translateUrl, traJson, `${lkey}`);
    }
  }
};
...
const toExcel = (url, translateUrl, filename, cb) => {
  ...
  const code = fs.readFileSync(
    path.join(process.cwd(), url.replace("/", path.sep)),
    "utf-8"
  );
  ...
  if (translateUrl) {
    traCode = fs.readFileSync(
      path.join(process.cwd(), translateUrl.replace("/", path.sep)),
      "utf-8"
    );
    traJson = jsonToData(
      traCode
        .slice(traCode.indexOf("{"), traCode.lastIndexOf("}") + 1)
        .replace(/\'/g, '"')
    );
  }
  let title = ["key", "子模块", "任务", "特性", "界面", "多级嵌套", "中文信息"];
  let data = [title];
  objToLink(data, json, translateUrl, traJson);
  let buffer = xlsx.build([{ name: "词条映射表", data }]);
...
const tojs = (url, filename, cb) => {
  ...
  try {
    ...
    code = `export default ${JSON.stringify(jsonData, "", 2).replace(
      /\"/g,
      "'"
    )}\r\n`;
