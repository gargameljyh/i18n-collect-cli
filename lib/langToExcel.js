  ...
  for (let lkey in obj) {
    if (typeof obj[lkey] === "string") {
      if (
        ...
        list.push([`${linkStr}_${lkey}`, "", "", "", "", "", obj[lkey]]);
      }
    }
    if (typeof obj[lkey] === "object") {
      objToLink(list, obj[lkey], translateUrl, traJson, `${linkStr}_${lkey}`);
    }
  }
};
...
const toExcel = (url, translateUrl, filename, cb) => {
  ...
  let data = [title];
  for (let key in json) {
    objToLink(data, json, translateUrl, traJson, key);
  }
