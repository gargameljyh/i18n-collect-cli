...
function readFileList(dir, src, pages, langObj, ignoredir) {
  ...
  filesList.forEach((fullPath) => {
    ...
    let path2 = fullPath.replace(
      path.join(process.cwd(), dirU + "src" + dirU),
      ""
    );
              ...
              if ([".vue"].includes(extname)) {
                ...
                if (isTemplate) {
                  ...
                  props.forEach((item) => {
                    if (
                      ...
                      let LangItem =
                        ":" + item.replace(str, `t('${langObj[str]}')`);
                ...
                if (
                  ...
                  ContentLine = ContentLine.replace(
                    `'${str}'`,
                    isTemplate ? `t('${langObj[str]}')` : `t('${langObj[str]}')`
                  );
                  ContentLine = ContentLine.replace(
                    `"${str}"`,
                    isTemplate
...
module.exports = (
  ...
  for (let cs_key in lang) {
    langObj[lang[cs_key]] = cs_key;
  }

  console.log(langObj);
  delDir(path.join(process.cwd(), dirU + src));
  readFileList(filePath, src, pages, langObj, ignoredir)
    .then(() => {
